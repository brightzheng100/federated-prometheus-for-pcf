...
# In PCF 1.12 this property belongs to 'autoscaling' but in 2.0 to 'clock_global'; in small footprint foundation it belongs to 'control'
doppler_url=$(jq -r '.instance_groups[] | select(.name == "clock_global" or .name == "autoscaling" or .name == "control") | .jobs[] | select (.name == "deploy-autoscaling") | .properties.doppler.host' < /tmp/cf-manifest.yml)
